...
class SafliiScraper(BaseScraper):
    ...
    def _indexing_sync(self) -> tuple[list[str], dict[str, str]]:
        ...
        with SB(uc=True, headless=self.headless, proxy=sb_proxy, multi_proxy=self.use_proxy, test=True) as sb:
            ...
            logger.info(f"Constructing year directory indices for court '{court_code}' across years {self.start_year}..{self.end_year}")
            year_links = [
                (y, f"https://www.saflii.org/za/cases/{court_code}/{y}/")
            ...
            while True:
                ...
                for attempt in range(1, 4):
                    try:
                        ...
                        if any(w in err_msg.lower() for w in ("invalid session id", "connection refused", "no such window", "target window already closed", "web view not found")):
                            logger.info(f"[Worker {worker_id}] Session connection disrupted. Executing UC reconnect recovery...")
                            try:
                                sb.uc_open_with_reconnect(case_url, reconnect_time=3)
                            except Exception:
                                pass
                        if attempt < 3:
                            sb.sleep(attempt * 3)
